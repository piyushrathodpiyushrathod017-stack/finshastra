...
export async function generateStaticParams() {
  ...
  return articles
    .filter((a) => a.category.slug === 'credit-score')
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/credit-score/${article.slug}`,
    },
    openGraph: {
      title: article.title,
      description: article.excerpt,
      url: `${SITE_URL}/credit-score/${article.slug}`,
      siteName: SITE_NAME,
      type: 'article',
...
export default async function CreditScoreArticlePage({ params }: { params: Promise<PageParams> }) {
  const { slug } = await params;
  const article = await getArticleBySlug(slug);

  if (!article || article.category.slug !== 'credit-score') {
    notFound();
  }
  ...
  const breadcrumbItems = [
    ...
    { label: article.title, href: `/credit-score/${article.slug}` },
  ];
