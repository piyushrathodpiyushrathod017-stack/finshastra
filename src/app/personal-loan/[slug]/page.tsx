...
export async function generateStaticParams() {
  ...
  return articles
    .filter((a) => a.category.slug === 'personal-loan')
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/personal-loan/${article.slug}`,
    },
    openGraph: {
      title: article.title,
      description: article.excerpt,
      url: `${SITE_URL}/personal-loan/${article.slug}`,
      siteName: SITE_NAME,
      type: 'article',
...
export default async function PersonalLoanArticlePage({ params }: { params: Promise<PageParams> }) {
  const { slug } = await params;
  const article = await getArticleBySlug(slug);

  if (!article || article.category.slug !== 'personal-loan') {
    notFound();
  }
  ...
  const breadcrumbItems = [
    ...
    { label: article.title, href: `/personal-loan/${article.slug}` },
  ];
